...
\journal{[-jrnl | replace("\\", "@BSL@") | replace("{", "\\{") | replace("}", "\\}") | replace("&", "\\&") | replace("%", "\\%") | replace("$", "\\$") | replace("#", "\\#") | replace("_", "\\_") | replace("~", "\\textasciitilde{}") | replace("^", "\\textasciicircum{}") | replace("@BSL@", "\\textbackslash{}")-]}
[# endif #]
...
%% must sit INSIDE the environment: elsarticle defines \JEL only there. They
%% come from the BUILT-IN `tags`, which is what tags are for.
[# set jel = doc.tags | join("; ") if doc.tags else '' #]
[# if doc.keywords or jel #]
[# if options.document_class == 'elsarticle' #]
